...
- **AI Assist tab** — Bug Fix Assist and Feature Suggestions are powered by
  **Claude** (Anthropic's API). Bug fixes always show you a before/after
...
- **Cloud tab:** Add a Claude API key (get one at
  https://console.anthropic.com) — this powers Bug Fix Assist and Feature
  Suggestions. A Gemini key is optional, only for the separate "Ask
  Gemini" box.

## AI Code Assist tab (🩹)

- **Bug Fix Assist (Claude)** — pick "Active project" or "Nexus itself,"
  pick a file, optionally paste an error, click **Analyze**. Claude
...
- **Feature Suggestions (Claude)** — reads your project's file list and
...
- **Git** — shows the active project's current branch and `git status`.
  ...
  remote repository.
- **Feature Builder (Claude)** — describe a feature in plain language and
  Claude proposes a plan: which files need to change or be created, and
...
- **This Project's Own API Keys / Secrets** — separate from Nexus's own
  Claude/Gemini keys. Add any key/value pair (e.g. `OPENAI_API_KEY`,
...
not just a reference doc: **every prompt Nexus sends to Claude for that
project — Bug Fix Assist, Feature Builder's plan step, Feature Builder's
per-file generation, and Feature Suggestions — includes it and is
instructed to follow it strictly**, or explain in its response why it
couldn't.
...
- API keys (Claude and Gemini) are stored encrypted using Windows' built-in
  credential encryption (via Electron's `safeStorage`), inside a small
  config file Nexus creates for itself — not in this project folder.
...
- **"npm run dev" doesn't do anything when you Launch a project** — check
  ...
  start.
- **Bug Fix Assist / Feature Suggestions say no key saved** — add a Claude
  API key in the Cloud tab first; those two features don't use Gemini.
