...
async function firebaseJson(url, body, options = {}) {
  const response = await fetch(url, { method: options.method || 'POST', headers: { 'Content-Type': 'application/json', ...(options.token ? { Authorization: `Bearer ${options.token}` } : {}) }, body: body === undefined ? undefined : JSON.stringify(body) });
...
async function signUp(apiKey, email, password) {
  return firebaseJson(`${AUTH_BASE}/accounts:signUp?key=${encodeURIComponent(apiKey)}`, { email, password, returnSecureToken: true });
}
...
async function sendPasswordReset(apiKey, email) {
  return firebaseJson(`${AUTH_BASE}/accounts:sendOobCode?key=${encodeURIComponent(apiKey)}`, { requestType: 'PASSWORD_RESET', email });
}

async function lookupAccount(apiKey, idToken) {
  const data = await firebaseJson(`${AUTH_BASE}/accounts:lookup?key=${encodeURIComponent(apiKey)}`, { idToken });
...
async function saveAccountVault({ apiKey, projectId, uid, idToken, encryptedVault }) {
  requireConfiguration(apiKey, projectId);
  const updatedAt = new Date().toISOString();
  await firebaseJson(firestoreDocumentUrl(projectId, uid), { fields: { encryptedVault: { stringValue: encryptedVault }, updatedAt: { timestampValue: updatedAt }, schemaVersion: { integerValue: '1' } } }, { method: 'PATCH', token: idToken });
  return { updatedAt };
}

async function loadAccountVault({ apiKey, projectId, uid, idToken }) {
  requireConfiguration(apiKey, projectId);
  const response = await fetch(firestoreDocumentUrl(projectId, uid), { headers: { Authorization: `Bearer ${idToken}` } });
