function installCruciblePluginUi(window) {
  window.webContents.executeJavaScript(`(() => {
    ...
    async function openGovernanceConfiguration() {
      ...
      const refresh = async () => {
        ...
        try {
          ...
          filePanel.innerHTML = files.map((file, index) => `<button class="btn tiny btn-secondary" data-crucible-file="${index}" style="display:block;width:100%;text-align:left;margin:4px 0;white-space:normal;">${String(file).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</button>`).join('') || '<p class="muted small">No governance files yet. Use New governance file or Auto Inject.</p>';
        ...
        } catch (error) { filePanel.innerHTML = '<p class="muted small">' + error.message + '</p>'; }
      };
    ...
    install();
  })();`).catch((error) => console.error('[Nexus] Crucible plugin UI failed:', error.message));
}

module.exports = { installCruciblePluginUi };
